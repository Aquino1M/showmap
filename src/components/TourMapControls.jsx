  ...
  return (
    <div className="absolute top-4 left-4 z-20 flex items-start gap-2">
      ...
      <div className="relative">
        <button onClick={() => setIsArtistMenuOpen((open) => !open)} aria-expanded={isArtistMenuOpen} className={`min-h-14 rounded-xl border px-3 text-xs font-bold shadow-lg backdrop-blur transition-colors ${selectedArtist ? 'border-cyan-400 bg-cyan-500 text-slate-950' : 'border-slate-700 bg-[#0B0F19]/90 text-indigo-300 hover:bg-indigo-600 hover:text-white'}`}>
